...
exports.default = function () {
    ...
    function init(container, planets) {
        ...
        _planets = planets;
        _scene = new _three2.default.Scene();
        _camera = createCamera(_scene);
        _renderer = createRenderer(_container);
        THREEx.WindowResize(_renderer, _camera);
        controls = new _three2.default.OrbitControls(_camera, _renderer.domElement);
    ...
    function createCamera(_scene) {
        ...
        var VIEW_ANGLE = 45,
            ...
            NEAR = 0.1,
            FAR = 20000;
        var camera = new _three2.default.PerspectiveCamera(VIEW_ANGLE, ASPECT, NEAR, FAR);
        _scene.add(camera);
    ...
    function renderScene() {
        _group = new _three2.default.Object3D();
        _revolving_planets = [];
        var planet, circle, planet_group;
        for (var i = 0; i < _planets.length; i++) {
            planet = Planet.createRandom(_planets[i], _planets[i].scale * MAX_SIZE.x, _planets[i].importance * MAX_RADIUS + MIN_RADIUS);
            circle = Planet.createPath(planet);
            planet_group = new _three2.default.Object3D();
    ...
    function setupRaycasting() {
        _raycaster = new _three2.default.Raycaster();
        _mouse = new _three2.default.Vector2();
    ...
    function animatePlanets() {
        var speed;
        for (var i = 0; i < _revolving_planets.length; i++) {
            if (INTERSECTED == null || INTERSECTED.userData.item.id !== _revolving_planets[i].children[0].userData.item.id) {
    ...
    function calculatePlanetSpeed(radius) {
        if (MAX_RADIUS == undefined) MAX_RADIUS = calculateMaxRadius();
...
Planet = function () {
    ...
    function createPath(planet) {
        var radius = Math.sqrt(Math.pow(planet.position.x, 2) + Math.pow(planet.position.y, 2)),
            segments = 64,
            material = new _three2.default.LineBasicMaterial({ color: 0xffffff }),
            geometry = new _three2.default.CircleGeometry(radius, segments);

        // Remove center vertex
        geometry.vertices.shift();

        // To get a closed path use LineLoop instead (see also @jackrugile his comment):
        var path = new _three2.default.LineLoop(geometry, material);
        path.userData = {
            radius: radius
        };
        return path;
    ...
    function createRandom(item, scale, importance) {
        ...
        return create(item, Math.random() * 0x808008 + 0x808080, position, scale);
    }

    function create(item, color, position, scale) {
        var geometry = new _three2.default.SphereGeometry(10, 10, 10);
        geometry.mergeVertices();
        var material = new _three2.default.MeshBasicMaterial({
            color: color
        });
        var planet = new _three2.default.Mesh(geometry, material);
